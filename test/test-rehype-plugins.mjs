...
function runTests() {
  ...
  {
    const tree = makeAnchorTree('page.md');
    transform(tree, STD_FILE, { ...STD_OPTS, base: '/GOMAD-METHOD/' });
    assert(getHref(tree) === '/GOMAD-METHOD/guide/page/', 'Base /GOMAD-METHOD/ -> /GOMAD-METHOD/guide/page/', `Got ${getHref(tree)}`);
  }

  {
    const tree = makeAnchorTree('page.md');
    transform(tree, STD_FILE, { ...STD_OPTS, base: '/GOMAD-METHOD' });
    assert(getHref(tree) === '/GOMAD-METHOD/guide/page/', 'Base /GOMAD-METHOD (no trailing slash) -> same result', `Got ${getHref(tree)}`);
  }
  ...
  {
    const tree = makeAnchorTree('/page/');
    transformBase(tree, { base: '/GOMAD-METHOD/' });
    assert(getHref(tree) === '/GOMAD-METHOD/page/', 'Base /GOMAD-METHOD/ prefixes', `Got ${getHref(tree)}`);
  }

  {
    const tree = makeAnchorTree('/page/');
    transformBase(tree, { base: '/GOMAD-METHOD' });
    assert(getHref(tree) === '/GOMAD-METHOD/page/', 'Base /GOMAD-METHOD normalizes (adds trailing slash)', `Got ${getHref(tree)}`);
  }
...

  {
    ...
    assert(getHref(tree) === '/GOMAD-METHOD/page/', 'a[href] prefixed', `Got ${getHref(tree)}`);
  }

  {
    const tree = makeElementTree('img', { src: '/img/logo.png' });
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/GOMAD-METHOD/img/logo.png', 'img[src] prefixed', `Got ${getSrc(tree)}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(
      tree.children[0].properties.href === '/GOMAD-METHOD/styles/main.css',
      'link[href] prefixed',
      `Got ${tree.children[0].properties.href}`,
    );
  }
...

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/GOMAD-METHOD/media/intro.mp4', 'video[src] prefixed', `Got ${getSrc(tree)}`);
  }

  {
    const tree = makeElementTree('audio', { src: '/media/clip.mp3' });
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/GOMAD-METHOD/media/clip.mp3', 'audio[src] prefixed', `Got ${getSrc(tree)}`);
  }

  {
    const tree = makeElementTree('iframe', { src: '/embed/widget' });
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/GOMAD-METHOD/embed/widget', 'iframe[src] prefixed', `Got ${getSrc(tree)}`);
  }
...

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/GOMAD-METHOD/media/alt.mp4', 'source[src] prefixed', `Got ${getSrc(tree)}`);
  }
  ...
  {
    const tree = makeAnchorTree('/GOMAD-METHOD/page/');
    transformBase(tree, { base: BASE });
    assert(getHref(tree) === '/GOMAD-METHOD/page/', 'Already prefixed skipped', `Got ${getHref(tree)}`);
  }
...

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getRawValue(tree) === '<img src="/GOMAD-METHOD/img/logo.png">', 'Raw img src rewritten', `Got ${getRawValue(tree)}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getRawValue(tree) === '<a href="/GOMAD-METHOD/page/">link</a>', 'Raw a href rewritten', `Got ${getRawValue(tree)}`);
  }
...

  {
    const tree = {
      ...
      children: [
        {
          type: 'raw',
          value: '<img src="/GOMAD-METHOD/img/logo.png">',
        },
      ],
    };
    transformBase(tree, { base: BASE });
    assert(getRawValue(tree) === '<img src="/GOMAD-METHOD/img/logo.png">', 'Raw already prefixed unchanged', `Got ${getRawValue(tree)}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(
      getRawValue(tree) === '<a href="/GOMAD-METHOD/page/"><img src="/GOMAD-METHOD/img/logo.png"></a>',
...

  {
    ...
    transformBase(tree, { base: BASE });
    const href = getHref(tree);
    assert(href === '/GOMAD-METHOD/guide/sibling/', './sibling.md through both -> no double prefix', `Got ${href}`);
  }

  {
    ...
    transformBase(tree, { base: BASE });
    assert(getSrc(tree) === '/GOMAD-METHOD/img/logo.png', 'img /img/logo.png -> only base-paths prefixes', `Got ${getSrc(tree)}`);
  }
...

  {
    ...
    transform(tree, STD_FILE, { ...STD_OPTS, base: BASE });
    transformBase(tree, { base: BASE });
    assert(getHref(tree) === '/GOMAD-METHOD/page/', '/page/ (non-.md) -> only base-paths prefixes', `Got ${getHref(tree)}`);
  }
