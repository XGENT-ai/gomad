...
test("full CRUD cycle", async ({ request }) => {
  ...
  const createResp = await request.post("/api/items", {
    data: { title: "Hammer", price: 19.99, category: "tools" },
  });
  ...
  const putResp = await request.put(`/api/items/${created.id}`, {
    data: { title: "Claw Hammer", price: 24.99, category: "tools" },
  });
...
Every API test should assert on status and body. Never skip these.
...
A missing 401 test today is a security hole tomorrow.
...
test("complete order workflow", async ({ request }) => {
  ...
  const orderResp = await request.post("/api/orders", {
    data: {
      cartId: cart.id,
      shippingAddress: { street: "456 Main Ave", city: "Metropolis", zip: "54321" },
    },
  });
...
test("API + E2E hybrid — seed via API, verify in browser", async ({
  ...
  const resp = await request.post("/api/products", {
    data: { name: `Hybrid Product ${Date.now()}`, price: 35.0, published: true },
  });
...
**Fix**: Use `webServer` in config to start it automatically.
...
**Cause**: The endpoint returned HTML (error page), plain text, or an empty body.

**Fix**: Check `response.status()` first — a 500 or 302 often returns HTML. Log `await response.text()` to see the actual body.
...
**Fix**: Set `extraHTTPHeaders` in config or create a custom authenticated fixture.
...
> **Critical**: GraphQL returns HTTP 200 even on errors. Always check both `data` and `errors` in the response body — `resp.ok()` alone is not sufficient.
...
test("mutation creates resource", async ({ request }) => {
  const resp = await request.post(GQL_ENDPOINT, {
    data: {
      ...
      `,
      variables: {
        input: { title: "New Widget", price: 15.0, status: "DRAFT" },
      },
    },
  });
...
test("handles validation errors", async ({ request }) => {
  const resp = await request.post(GQL_ENDPOINT, {
    data: {
      query: `mutation AddItem($input: ItemInput!) { addItem(input: $input) { id } }`,
...
test("handles authorization errors", async ({ request }) => {
  const resp = await request.post(GQL_ENDPOINT, {
    data: {
      query: `query AdminDashboard { adminMetrics { revenue activeUsers } }`,
    },
  });
...
Extract queries to avoid duplication across test files:
...
**GraphQL returns 200 but data is null**: The actual error is in the `errors` array.
...
**"Cannot query field X on type Y"**: Use introspection to debug the schema.
...
**Variables not being applied**: Variable names in the query must match the `variables` object keys exactly (case-sensitive).
