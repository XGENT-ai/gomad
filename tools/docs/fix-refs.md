...
description: Corrects workflow, agent, and command references in GoMad documentation
---
...
Fix incorrect references to workflows, agents, and commands in GoMad documentation files.
...
**How-To guides require judgment**: Don't assume experienced. Ask: "Does this task require prior GoMad knowledge?" Early-journey tasks (first PRD, first sprint) are newbie docs. Customization and advanced features are experienced.

**If unclear**: Ask the user "Who is the target audience for this document — new users learning GoMad, or experienced users who know the system?"
...
| `gomad_gomad_*` | `gomad_gomad_workflow-init` | Internal slash command name, platform-specific |
...
- First mention: Run the `help` workflow (`gomad-help` on most platforms)
...
|----------|----------|-------|
| `workflow-init` | `gomad-help` | DEPRECATED - help system replaces initialization |
| `workflow-status` | `gomad-help` | DEPRECATED - help system replaces status checking |

### The Help System

The `gomad-help` workflow is the modern replacement for both `workflow-init` and `workflow-status`:
...
Users can run `gomad-help` to get guidance on what to do next. It detects:
...
3. **Simple names**: Just the workflow name, no `gomad_gomad_` prefix, no `/` prefix
