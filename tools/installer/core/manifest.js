...
class Manifest {
   ...
   */
  async create(gomadDir, data, installedFiles = []) {
    ...
    // Get the GoMad version from package.json
    const gomadVersion = data.version || require(path.join(process.cwd(), 'package.json')).version;
    ...
    if (data.modules && Array.isArray(data.modules)) {
      for (const moduleName of data.modules) {
        ...
        moduleDetails.push({
          ...
          installDate: now,
          lastUpdated: now,
          source: moduleName === 'core' || moduleName === 'gomad' ? 'built-in' : 'unknown',
        });
      }
    }
   ...
   */
  async read(gomadDir) {
    const yamlPath = path.join(gomadDir, '_config', 'manifest.yaml');
    const yaml = require('yaml');
   ...
   */
  async update(gomadDir, updates, installedFiles = null) {
    const yaml = require('yaml');
    const manifest = (await this._readRaw(gomadDir)) || {
      installation: {},
      modules: [],
      ides: [],
    };
   ...
   */
  async addModule(gomadDir, moduleName, options = {}) {
    let manifest = await this._readRaw(gomadDir);
    if (!manifest) {
    ...
    await this._writeRaw(gomadDir, manifest);
  }
   ...
   */
  async removeModule(gomadDir, moduleName) {
    const manifest = await this._readRaw(gomadDir);
   ...
   */
  async updateModuleVersion(gomadDir, moduleName, versionInfo) {
    const manifest = await this._readRaw(gomadDir);
    if (!manifest || !manifest.modules) {
      return;
    }
   ...
   */
  async getModuleVersion(gomadDir, moduleName) {
    const manifest = await this._readRaw(gomadDir);
    if (!manifest || !manifest.modules) {
      return null;
    }
   ...
   */
  async getAllModuleVersions(gomadDir) {
    const manifest = await this._readRaw(gomadDir);
    if (!manifest || !manifest.modules) {
      return [];
    }
   ...
   */
  async _writeRaw(gomadDir, manifestData) {
    const yaml = require('yaml');
    const manifestPath = path.join(gomadDir, '_config', 'manifest.yaml');
   ...
   */
  async addIde(gomadDir, ideName) {
    const manifest = await this.read(gomadDir);
    if (!manifest) {
    ...
    if (!manifest.ides.includes(ideName)) {
      ...
      await this.update(gomadDir, { ides: manifest.ides });
    }
  }
   ...
   * @param {string} gomadDir - Path to gomad directory for relative paths
   * @returns {Array} Array of file metadata objects
   */
  async parseInstalledFiles(installedFiles, gomadDir) {
    const fileMetadata = [];

    for (const filePath of installedFiles) {
      const fileExt = path.extname(filePath).toLowerCase();
      // Make path relative to parent of gomad directory, starting with 'gomad/'
      const relativePath = 'gomad' + filePath.replace(gomadDir, '').replaceAll('\\', '/');
   ...
   * @param {string} relativePath - Relative path starting with 'gomad/'
   ...
   */
  async addCustomModule(gomadDir, customModule) {
    const manifest = await this.read(gomadDir);
    if (!manifest) {
      throw new Error('No manifest found');
    }
    ...
    await this.update(gomadDir, { customModules: manifest.customModules });
  }
   ...
   */
  async removeCustomModule(gomadDir, moduleId) {
    const manifest = await this.read(gomadDir);
   ...
   */
  async getModuleVersionInfo(moduleName, gomadDir, moduleSourcePath = null) {
    const os = require('node:os');
    const yaml = require('yaml');

    // Built-in modules use GoMad version (only core and gomad are in GOMAD-METHOD repo)
    if (['core', 'gomad'].includes(moduleName)) {
      const gomadVersion = require(path.join(getProjectRoot(), 'package.json')).version;
      return {
        version: gomadVersion,
        source: 'built-in',
        npmPackage: null,
    ...
    const cacheDir = path.join(gomadDir, '_config', 'custom', moduleName);
   ...
   */
  async checkForUpdates(gomadDir) {
    const modules = await this.getAllModuleVersions(gomadDir);
    const updates = [];
