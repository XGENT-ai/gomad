...
- 🚧 **v1.3 Marketplace, Docs & Story Context** — Phases 10-13 (in progress)
...
Decimal phases appear between their surrounding integers in numeric order. Phase numbering continues from v1.2 (ended at Phase 9) — v1.3 starts at Phase 10.
...
### 🚧 v1.3 Marketplace, Docs & Story Context (Phases 10-13)

**Milestone Goal:** Refresh public surface (plugin marketplace + docs site), restructure the agent install path, and enrich the 4-implementation story workflow with a discuss step plus a domain-knowledge framework. Ship as `@xgent-ai/gomad@1.3.0` with explicit BREAKING callout for the agent-dir relocation.

**Structure:** 4 phases, coarse granularity. Phases 10/11/12 are independent and theoretically parallel, but the shipping order matters: marketplace lands first (regression harness), story-creation lands second (exercises `_config/<subdir>/` greenfield path as warm-up for relocation), docs content authored third (references the post-v1.3 install layout), then Phase 13 executes the agent-dir relocation with exclusive lock — simultaneously shipping the release (CHANGELOG BREAKING + `npm publish` + `v1.3.0` tag). The 4-phase plan (not 5) is justified because AGENT-* and REL-* are so tightly coupled — REL-02 quality gate depends on every AGENT-* test, AGENT-10 extends `verify-tarball.js` which is the release gate itself, and CHANGELOG BREAKING (REL-01) is specifically about the agent-dir move. Splitting them would manufacture an artificial boundary.

- [ ] **Phase 10: Marketplace Refresh** — Full BMAD→GoMad rewrite of `.claude-plugin/marketplace.json` with 3-plugin grouping (`gomad-agents` / `gomad-skills` / `gomad-core`), identity match to `@xgent-ai/gomad@1.3.0`, and `test:marketplace` CI gate.
- [ ] **Phase 11: Story-Creation Enhancements** — Ship `gm-discuss-story` + `gm-domain-skill` under `4-implementation/`, 2 seed KB packs in `src/domain-kb/`, installer `_installDomainKb()` step copying to `<installRoot>/_config/kb/`, context auto-load in `gm-create-story`, and `validate-kb-licenses.js` gate.
- [ ] **Phase 12: Docs Site Content Authoring** — Author Install / Quick-Start / Agents / Skills / Architecture / Contributing pages plus zh-cn parity under `docs/**`; Starlight auto-deploy pipeline (already shipped in v1.2) handles publishing to `gomad.xgent.ai`.
- [ ] **Phase 13: Agent Dir Relocation + Release** — Relocate persona bodies `<installRoot>/gomad/agents/` → `<installRoot>/_config/agents/` across 11 touchpoints, fix `newInstallSet` latent bug, tweak custom-file detector, add v1.2→v1.3 upgrade test, finalize DOCS-07 path sweep, publish `@xgent-ai/gomad@1.3.0` with BREAKING callout.

## Phase Details

### Phase 10: Marketplace Refresh
**Goal**: Plugin marketplace file matches v1.3 package identity with clean, validated 3-group structure that Claude Code can ingest without error.
**Depends on**: Nothing (v1.2 baseline; independent of all other v1.3 workstreams)
**Requirements**: MARKET-01, MARKET-02, MARKET-03, MARKET-04, MARKET-05
**Success Criteria** (what must be TRUE):
  1. Claude Code user invoking `/plugin install @xgent-ai/gomad` (or browsing the marketplace) sees 3 plugin groups listed — `gomad-agents`, `gomad-skills`, `gomad-core` — with identity fields (owner, homepage, repository, license) reflecting xgent-ai and zero BMAD-era strings.
  2. Every `skills:` / `agents:` path declared in `marketplace.json` resolves to a real source directory containing a readable `SKILL.md` — validated programmatically by `test:marketplace`.
  3. `plugins[].version` reads `1.3.0` (matching `package.json`); CI fails if the two drift.
  4. Developer running `npm run quality` sees `test:marketplace` exit 0; intentionally breaking any path (e.g., typo) makes the gate fail loudly.
  5. `claude plugin validate .` exits 0 against the refreshed file.
**Plans**: TBD

### Phase 11: Story-Creation Enhancements
...
### Phase 12: Docs Site Content Authoring
...
**Depends on**: Nothing for content authoring (Starlight pipeline already shipped in v1.2); soft dependency — path examples use the canonical post-v1.3 layout (`<installRoot>/_config/agents/`), which Phase 13 formally validates via DOCS-07.
...
### Phase 13: Agent Dir Relocation + Release
...
**Depends on**: Phase 10 (marketplace harness green as regression coverage for path changes), Phase 11 (proven `_config/<subdir>/` greenfield install pattern before touching collision-prone `_config/agents/`), Phase 12 (doc content ready for path-sweep finalization via DOCS-07).
...
**Success Criteria** (what must be TRUE):
  ...
  3. Developer running `npm run quality` on the release commit sees exit 0 — `test:marketplace` + `test:gm-surface` (extended with launcher-body regex) + `test:tarball` (extended with legacy-path grep) + `test:legacy-v12-upgrade` + `test:v13-agent-relocation` + `test:domain-kb-install` + `test:prd-chain` + `docs:build` + `validate-kb-licenses` + `tools/validate-doc-paths.js` all pass.
  4. Developer reading `CHANGELOG.md` for `1.3.0` sees an explicit `### BREAKING CHANGES` section documenting the agent-dir move with old-path → new-path migration instructions and a backup-recovery cross-reference; `docs/upgrade-recovery.md` reflects v1.2 → v1.3 migration steps; all docs path examples (from Phase 12) use the canonical post-v1.3 layout with zero `gomad/agents/` leaks (verified by `tools/validate-doc-paths.js`).
...
**Plans**: TBD
**Research flag**: yes — `/gsd-research-phase` recommended before `/gsd-plan-phase 13`. Two open design questions have cascading effects: (a) `newInstallSet` bug fix (AGENT-04) — choose between deriving `newInstallSet` from current install's planned outputs vs. adding relocation-specific branch to `buildCleanupPlan` analogous to `isV11Legacy`; (b) `_config/agents/` collision resolution (AGENT-05) — Option 2 (extend custom-file detector to treat matching `.md` as generated) is the smallest-diff path per research, but confirm before planning.

## Progress

**Execution Order:**
Phases execute in numeric order: 10 → 11 → 12 → 13 (Phases 10/11/12 are logically independent; recommended shipping order reflects risk-minimization — marketplace first locks regression harness, story-creation second exercises `_config/` subdir pattern, docs content third is authored after the story skills exist to reference, Phase 13 last with exclusive lock because it's simultaneously the riskiest change and the release itself).
...
| 10. Marketplace Refresh                            | v1.3      | 0/TBD          | Not started | -          |
| 11. Story-Creation Enhancements                    | v1.3      | 0/TBD          | Not started | -          |
| 12. Docs Site Content Authoring                    | v1.3      | 0/TBD          | Not started | -          |
| 13. Agent Dir Relocation + Release                 | v1.3      | 0/TBD          | Not started | -          |

## Coverage

**v1.3 requirement coverage:** 39/39 mapped to phases ✓

| Phase | Requirements | Count |
|-------|--------------|-------|
| 10    | MARKET-01, MARKET-02, MARKET-03, MARKET-04, MARKET-05 | 5 |
| 11    | STORY-01, STORY-02, STORY-03, STORY-04, STORY-05, STORY-06, STORY-07, STORY-08, STORY-09, STORY-10, STORY-11, STORY-12 | 12 |
| 12    | DOCS-01, DOCS-02, DOCS-03, DOCS-04, DOCS-05, DOCS-06 | 6 |
| 13    | DOCS-07, AGENT-01, AGENT-02, AGENT-03, AGENT-04, AGENT-05, AGENT-06, AGENT-07, AGENT-08, AGENT-09, AGENT-10, AGENT-11, REL-01, REL-02, REL-03, REL-04 | 16 |
| **Total** | | **39** |

**Coverage notes:**

- **DOCS-07 assignment**: Placed entirely in Phase 13 (not split). Rationale: the requirement's enforcement mechanism (`tools/validate-doc-paths.js` wired into `npm run quality`) validates against real v1.3 paths, which requires AGENT-* to have landed. Phase 12 authors docs content using the canonical post-v1.3 layout (`<installRoot>/_config/agents/`) as documented-target; Phase 13 ships the linter and finalizes any sweep needed. Cleaner traceability — no REQ-ID split across phases.
...
- **Phase 13 exclusive lock**: No work overlaps with Phase 13 because it touches the installer's runtime pointer (`agent-command-template.md:16`), cleanup-planner semantics (`cleanup-planner.js:282`), and the release gate itself. Concurrent changes risk silent drift.
