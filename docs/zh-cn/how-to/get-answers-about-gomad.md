---
title: "如何获取关于 GoMad 的答案"
description: 使用 LLM 快速回答您自己的 GoMad 问题
sidebar:
  order: 4
---
...
**获取 GoMad 相关答案最快的方式是 `gomad-help` 技能。** 这个智能向导可以覆盖 80% 以上的常见问题，并且你在 IDE 里随时可用。

GoMad-Help 不只是查表工具，它还能：
- **检查你的项目状态**，判断哪些步骤已经完成
...
:::tip
按平台不同，你也可以使用 `/gomad-help` 或 `$gomad-help`。但大多数情况下直接输入 `gomad-help` 就能工作。
:::

也可以结合自然语言问题一起调用：

```
gomad-help 我有一个 SaaS 想法并且已经知道主要功能，我该从哪里开始？
gomad-help 我在 UX 设计方面有哪些选项？
gomad-help 我卡在 PRD 工作流了
gomad-help 帮我看看目前完成了什么
```
...
当你遇到以下情况时，可用本指南补充：
- 想理解 GoMad 的架构设计或内部机制
- 需要超出 GoMad-Help 覆盖范围的答案
- 在安装前做技术调研
- 想直接基于源码进行追问
...
:::note[示例]
**问：** “用 GoMad 做一个需求到实现的最短路径是什么？”

**答：** 使用 Quick Flow，运行 `gomad-quick-dev`。它会在一个工作流里完成意图澄清、计划、实现、审查与结果呈现，跳过完整规划阶段。
:::
